...
cmd({ pattern: "slanime", react: '📑', category: "movie", desc: "slanimeclub movie downloader", filename: __filename }, async (conn, m, mek, { from, prefix, q, l, reply }) => { try { if (!q) return await reply('Please Give Me Text..! 🖊️')
...
cmd({ pattern: 'slanimedl', react: "📥", dontAddCommandList: true, filename: __filename }, async (conn, mek, m, { from, q, reply }) => { if (!q) return await reply('Please provide a direct URL!')
...
try {
    ...
    if (dl_link.endsWith('.mp4') || dl_link.includes('slanimeclub.co')) {
